...
>(function Select({ className, children, ...props }, ref) {
  return (
    <select
      ref={ref}
      className={cn(
        'w-full rounded border border-border bg-surface px-3 h-9 text-[13px] text-foreground transition-colors focus:outline-none focus:border-accent appearance-none bg-no-repeat',
        className,
      )}
      style={{
        backgroundImage:
